...
# creating the parent key of the dictmodel
        ...
        for parent in parents:
            parent=Databaseobj.getParents(parent['uid'])
            j.write(" Parent: {}, UID: {}, Test:{}".format(parent['name'],parent['uid'],parent['test']))
            dictmodel['Parents'][parent['uid']]={}
            dictmodel['Parents'][parent['uid']]['childlist']={}
            dictmodel['Parents'][parent['uid']]['test']=parent['test']
            dictmodel['Parents'][parent['uid']]['name']=parent['name']
            dictmodel['Parents'][parent['uid']]['type']= parent['type']
            dictmodel['Parents'][parent['uid']]['uid']= parent['uid']
            for Parent_sections in parent['parent_sections']:
                for child_list in Parent_sections['child_lists']:
                    for child_member in child_list['childmembers']:
                        ...
                        dictmodel['Parents'][parent['uid']]['childlist'][child_member['memberid']]\
                            =Databaseobj.getChilds(child_member['memberid'])['name']

            j.write("Total number of childs {}\n\n".format(len(dictmodel['Parents'][parent['uid']]['childlist'])))
...
# creating the Totals key and calculating the totals
        ...
        TotalChilds=len(dictmodel['Childs'])
        j.write("Total number of Childs: {} \n".format(TotalChilds))
        dictmodel['Totals']['TotalChilds']=TotalChilds
                    ...
                    if isinstance(d1[k], dict):
                        ...
                        if all([_ in d1[k] for _ in ['name', 'uid', 'childlist', 'test']]):
                            ...
                            if len(d1[k]['childlist']) != len(d2[k]['childlist']):
                                ...
                                # track all parents with modified child lists for the summary report
        ...
        for checkType in sorted(d1.keys()):
            if checkType == "Totals":
                ...
                try:
                    ...
                    log.error('WARNING: No callable function for {}, encountered error {}'.format(checkType,e))
...
def Summary(d1,d2,file=None):
    ...
    try:
        ...
        log.info("The dictionaries do not contain the 'version' key")

    log.info("Creating the summary report for the differences between {} and {}".format(d1Version, d2Version))
...
# calculating the exact diff between the added, removed and changed keys from the Child summary
        ...
        f.write(
            "\nThe total number of added childs: {}, changed: {}, removed: {}.\n\n".format(
